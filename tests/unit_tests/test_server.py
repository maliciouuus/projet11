...
Ce module teste les fonctionnalités de base du serveur, notamment :
- le chargement et la sauvegarde des données,
- la vérification des compétitions ouvertes.
"""
...
def test_load_clubs():
    ...
    assert all(
        isinstance(club["points"], int) for club in clubs
    )  # Vérifie la conversion des points
...
def test_load_competitions():
    ...
    assert all(
        isinstance(comp["numberOfPlaces"], int) for comp in competitions
    )  # Vérifie la conversion des places


def test_save_clubs(tmp_path):
    """
    Test de sauvegarde des clubs.
    AMÉLIORATION: Vérifie que les points sont correctement reconvertis en chaînes
    pour le JSON.
    """
    ...
    with open("clubs.json") as f:
        saved = json.load(f)
    assert isinstance(
        saved["clubs"][0]["points"], str
    )  # Vérifie que les points sont stockés comme chaînes


def test_save_competitions(tmp_path):
    """
    Test de sauvegarde des compétitions.
    AMÉLIORATION: Vérifie que les places sont correctement reconverties en chaînes
    pour le JSON.
    """
    test_comps = [
        {"name": "Test Competition", "date": "2024-01-01", "numberOfPlaces": 20}
    ]
    saveCompetitions(test_comps)
    with open("competitions.json") as f:
        saved = json.load(f)
    assert isinstance(
        saved["competitions"][0]["numberOfPlaces"], str
    )  # Vérifie que les places sont stockées comme chaînes
